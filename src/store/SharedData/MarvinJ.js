...
export function MarvinImage(width, height, colorModel){
	// properties
	this.image = null;
	this.canvas = null;
	this.ctx=null;
...
MarvinImage.dataURItoBlob = function(dataURI) {
	...
	function MarvinImageMask (w, h){
		...
		if(w != 0 && h != 0){
			this.arrMask = MarvinJSUtils.createMatrix2D(w, h);
		} else{
			this.arrMask = null;
		}
	};
	...
	MarvinImageMask.prototype.clear = function(width,height){
...
MarvinAbstractImagePlugin.getAttribute = function(label, value){
	...
	FloodfillSegmentation.prototype.blobSegments = function(image, segments){
		...
		for(var i=0; i<segments.length; i++){
			...
			blobSegments[i] = new Marvin.BlobSegment(seg.x1, seg.y1);
			var tempBlob = new Marvin.Blob(seg.width, seg.height);
...
MarvinPoint.prototype.setY = function(y){
	this.y = y;
};
