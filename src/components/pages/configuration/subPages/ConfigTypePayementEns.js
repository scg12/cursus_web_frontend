...
import AddTypePayementEns from "../modals/AddTypePayementEns";
import { alpha, styled } from '@mui/material/styles';
import { DataGrid, gridClasses } from '@mui/x-data-grid';
import { useTranslation } from "react-i18next";


var payements = [], type_ens = [];  

function ConfigTypePayementEns(props) {
...
/*************************** DataGrid Declaration ***************************/    
    const columns = [
        ...
        {
            field: 'libelle_ens',
            headerName: 'Type Enseignant',
            width: 110,
            editable: false,
            headerClassName:classes.GridColumnStyle
        },
        {
            field: 'id_type_enseignant',
            headerName: 'num',
            width: 100,
            editable: false,
            hide:true,
...
/*************************** Handler functions ***************************/
    ...
    function getFormData(){
        ...
        payements.id_type_ens = document.getElementById('type_ens').value;
    ...
    function handleEditRow(row){       
        ...
        inputs[1]= row.id_type_enseignant;
        inputs[2]= row.id;
        inputs[3]= row.libelle_ens;
    ...
    function listPayements(){
        ...
        type_ens=[];
        axiosInstance
        .post(`list-type-payement-enseignants/`,{id_sousetab: currentAppContext.currentEtab,
            id_type_enseignant:""}).then((res)=>{
            res.data.payements.map((payement)=>{payements.push(payement)});
            res.data.type_ens.map((t)=>{type_ens.push(t)});
    ...
    function addNewPayement(e) {       
        ...
        var paymt = getFormData();
            ...
            axiosInstance.post(`create-type-payement-enseignant/`, {
                    id:0, 
                    libelle:paymt.libelle_payement,
                    id_type_enseignant:paymt.id_type_ens, 
    ...
    function modifyPayement(e) {
        ...
        console.log(paymt);
            ...
            axiosInstance.post(`update-type-payement-enseignant/`, {
                id:paymt.id, 
                libelle:paymt.libelle_payement,
                id_type_enseignant:paymt.id_type_ens, 
                montant:paymt.montant, 
    ...
    function deleteRow(rowId) {
        ...
        if(window.confirm('Voulez-vous vraiment supprimer le payement selectionné?')){
            ...
            axiosInstance
            .post(`delete-type-payement-enseignant/`, {
                id:rowId,
                id_sousetab: currentAppContext.currentEtab
            }).then((res)=>{
    ...
    return (
        <div className={classes.formStyle}>
            <div className={classes.inputRowLeft} style={{color:'rgb(6, 146, 18)', fontFamily:'Roboto, sans-serif', fontWeight:570, fontSize:'1.27vw', borderBottomStyle:'solid', borderBottomColor:'rgb(6, 146, 18)', borderBottomWidth:1.97, marginBottom:'1.3vh'}}> 
                {t("conf_types_paiements_prof")}
            </div>
            {(modalOpen!=0) && <AddTypePayementEns formMode= {(modalOpen==1) ? 'creation': 'modif'}  actionHandler={(modalOpen==1) ? addNewPayement : modifyPayement} cancelHandler={quitForm} />}

            {(modalOpen==0) ?
                <div className={classes.gridTitleRow}> 
                    <div className={classes.gridTitle}>
                        TYPES PAYEMENTS ENSEIGNANTS
                    </div>
...
export default ConfigTypePayementEns;