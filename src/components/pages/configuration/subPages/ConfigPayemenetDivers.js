...
import AddPayementDivers from "../modals/AddPayementDivers";
...
function ConfigPayementDivers(props) {
...
/*************************** Handler functions ***************************/
    ...
    function handleEditRow(row){       
        ...
        inputs[1]= row.montant;
        inputs[3]= row.entree_sortie_caisee;
        inputs[2]= row.id;
    ...
    function listPayements(){
        ...
        payements=[];
        axiosInstance
        .post(`list-type-payement-divers/`,{id_sousetab: currentAppContext.currentEtab}).then((res)=>{
    ...
    function addNewPayement(e) {       
        ...
        var paymt = getFormData();
            console.log(paymt);
            axiosInstance.post(`create-type-payement-divers/`, {
                id: paymt.id,
                libelle: paymt.libelle,
    ...
    function modifyPayement(e) {
        ...
        console.log(paymt);
            ...
            axiosInstance.post(`update-type-payement-divers/`, {
                id: paymt.id,
                libelle: paymt.libelle,
                montant: paymt.montant,
                e_s: paymt.e_s,
    ...
    function deleteRow(rowId) {
        ...
        if(window.confirm('Voulez-vous vraiment supprimer la section selectionnée?')){
            ...
            axiosInstance
            .post(`delete-type-payement-divers/`, {
                id:rowId,
                id_sousetab: currentAppContext.currentEtab
            }).then((res)=>{
    ...
    return (
        <div className={classes.formStyle}>
            <div className={classes.inputRowLeft} style={{color:'rgb(6, 146, 18)', fontFamily:'Roboto, sans-serif', fontWeight:570, fontSize:'1.27vw', borderBottomStyle:'solid', borderBottomColor:'rgb(6, 146, 18)', borderBottomWidth:1.97, marginBottom:'1.3vh'}}> 
                {t("conf_types_paiements_autres")}
            </div>
            {(modalOpen!=0) && <AddPayementDivers formMode= {(modalOpen==1) ? 'creation': 'modif'}  actionHandler={(modalOpen==1) ? addNewPayement : modifyPayement} cancelHandler={quitForm} />}

            {(modalOpen==0) ?
                <div className={classes.gridTitleRow}> 
                    <div className={classes.gridTitle}>
                        LISTE DES PAYEMENTS DIVERS
                    </div>
...
export default ConfigPayementDivers;